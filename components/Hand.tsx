...
import { IHandStates, IHandPart, IPartState, IHandProps, IHandSide } from "../interfaces/interface";

const fillState = (state: IPartState) => Object.keys(IHandPart)
  .reduce((acc, key) => ((acc[key] = state), acc), {} as IHandStates);

export const Hand: NextPage<IHandProps> = (props) => {
  const state: IHandStates = { ...fillState(IPartState.HIDE), ...props.state };
  ...
  const svgClass = cn("w-[281px]", {
    "-scale-x-100": props.side === IHandSide.RIGHT,
  });
  ...
  return (
    ...
    >
      <path
        className={stateMapping[state.F1]}
        id="F1"
      ...
      />
      <path
        className={stateMapping[state.F2]}
        id="F2"
      ...
      />
      <path
        className={stateMapping[state.F3]}
        id="F3"
      ...
      />
      <path
        className={stateMapping[state.F4]}
        id="F4"
      ...
      />
      <path
        className={stateMapping[state.F5]}
        id="f5"
      ...
      />
      <path
        className={stateMapping[state.PM]}
        id="PM"
